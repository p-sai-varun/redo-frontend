...
import PostJob from './PostJob';
  ...
  const [tag, settag] = useState("company");
    ...
    return (
        <div>
            {location.state=='poster'?<PostJob/>: <div> <input
            type="text"
...
< h2 className="text-white">Companies hiring right now</h2>
            <br/>
            {allJobs.length > 0 ? allJobs.map((item)  =>  <Job userId={location.id} singleJob={item} updateArray={removeDeletedJob} />) : <div className="text-white">No Job Openings</div>}
            <ToastContainer/></div>
            }
        </div>
    );
